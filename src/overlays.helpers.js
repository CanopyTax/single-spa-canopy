  ...
  window.addEventListener("keypress", function (evt) {
    if ((evt.key === "~" || evt.key === "`") && evt.shiftKey && evt.ctrlKey) {
      localStorage.setItem(
        "cp:single-spa:overlay",
        !JSON.parse(localStorage.getItem("cp:single-spa:overlay")),
      );
...
function canDevOverlayBeTurnedOn() {
  if (typeof localStorage === "undefined") {
    ...
    return (
      localStorage.getItem("sofe-inspector") === "true" ||
      localStorage.getItem("cp:dev-overlay") === "true"
    );
  }
}
...
export function setOrRemoveAllOverlays(rootElement, opts, props) {
  const overlayEnabled =
    localStorage.getItem("cp:single-spa:overlay") === "true";

  clearInterval(opts.overlay._interval);
  if (overlayEnabled) {
    opts.overlay._interval = setInterval(() => {
      immediatelySetOrRemoveAllOverlays(
        rootElement,
        opts,
        props,
        overlayEnabled,
...
function immediatelySetOrRemoveAllOverlays(
  rootElement,
  opts,
  props,
  overlayEnabled,
) {
  setOrRemoveOverlay(rootElement, overlayEnabled, opts, props, [
    overlayDivClassName,
    "rootElement",
  ]);
  const selectorNodeLists = opts.overlay.selectors.map((selector) =>
    rootElement.querySelectorAll(selector),
  );

  selectorNodeLists.forEach((selectorNodeList) => {
    for (let i = 0; i < selectorNodeList.length; i++) {
      setOrRemoveOverlay(selectorNodeList[i], overlayEnabled, opts, props, [
        overlayDivClassName,
      ]);
    }
  });
...
function setOrRemoveOverlay(element, overlayEnabled, opts, props, classes) {
  ...
  if (overlayEnabled) {
    ...
    if (
      overlayDiv.parentElement.clientHeight > 0 &&
      overlayDiv.parentElement.clientWidth > 0
    ) {
...
function createOverlayWithText(opts, props, elementToAppendChild, classes) {
  ...
  childDiv.style.flexDirection =
    elementToAppendChild.clientHeight > 80 ? "column" : "row";
  childDiv.style.alignItems = "center";
  childDiv.style.justifyContent = "center";
  childDiv.style.color =
    opts.overlay.color ||
    opts.overlay.textColor ||
    getColorFromString(getAppName(props), 1);
  ...
  import("@canopytax/error-logging")
    .then((sentry) => {
      if (typeof sentry.serviceNameToSquad === "function") {
        ...
        squadDiv.appendChild(
          document.createTextNode(
            `(${sentry.serviceNameToSquad(getAppName(props))} squad)`,
          ),
        );
